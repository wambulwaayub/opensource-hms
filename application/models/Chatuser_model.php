...
class Chatuser_model extends CI_Model
{
    ...
    {
        $sql = "SELECT chat_users.id as `chat_user_id`, chat_users.user_type,chat_users.patient_id,staff_id,(CASE WHEN staff_id IS NULL THEN (SELECT patients.patient_name as `name` FROM patients WHERE patients.id=chat_users.patient_id) ELSE (SELECT (CASE WHEN staff.surname != '' THEN CONCAT(staff.name, ' ',staff.surname) ELSE staff.name END) as `name` FROM staff WHERE staff.id=chat_users.staff_id) END) as name,(CASE WHEN staff_id IS NULL THEN (SELECT
patients.image as `image` FROM patients WHERE patients.id=chat_users.patient_id) ELSE (SELECT staff.image as `image` FROM staff WHERE staff.id=chat_users.staff_id) END) as image FROM `chat_users` WHERE chat_users.id=" . $chat_user_id;
    ...
    public function addNewUserForStudent($first_entry, $insert_data, $panel = "patient", $id, $insert_message)
    {
        $chat_connections = array('chat_user_one' => '', 'chat_user_two' => '');
