...
class Customlib {
    ...
    function getHospitalCurrencyFormat() {
        $admin = $this->CI->session->userdata('hospitaladmin');
        if ($admin) {
            return $admin['currency_symbol'];
    ...
    function getStudentSessionUserID() {
        ...
        $patientID = $session_Array['patient'];
        return $patientID;
    }
     ...
     public function sendMailSMS($find)
    {
        ...
        if (!empty($notifications)) {
            foreach ($notifications as $note_key => $note_value) {
                ...
                return array('mail' => $note_value->is_mail, 'sms' => $note_value->is_sms, 'mobileapp' => $note_value->is_notification, 'template' => $note_value->template, 'subject' => $note_value->subject);
