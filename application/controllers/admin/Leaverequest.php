...
class Leaverequest extends Admin_Controller
{
    ...
    public function addLeave()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $document = rand(10,100). '.' . $fileInfo['extension'];
            ...
            if (!empty($request_id)) {
                $data = array('id' => $request_id,
                    ...
                    'leave_to'         => $leaveto, 'employee_remark' => $reason, 'status' => $status, 'admin_remark' => $remark, 'applied_by' => $applied_by);
                ...
                if (isset($_FILES["userfile"]) && !empty($_FILES['userfile']['name'])) {
                    ...
                    $data_img = array('id' => $request_id, 'document_file' => $img_name);
                ...
                $data = array('staff_id' => $staff_id, 'date' => date('Y-m-d', $this->customlib->datetostrtotime($applied_date)), 'leave_days' => $leave_days, 'leave_type_id' => $leavetype, 'leave_from' => $leavefrom, 'leave_to' => $leaveto, 'employee_remark' => $reason, 'status' => $status, 'admin_remark' => $remark, 'applied_by' => $applied_by);

                $insert_id = $this->leaverequest_model->addLeaveRequest($data);
                ...
                if (isset($_FILES["userfile"]) && !empty($_FILES['userfile']['name'])) {
                    ...
                    $img_name = $insert_id . '.' . $fileInfo['extension'];
                    move_uploaded_file($_FILES["userfile"]["tmp_name"], "./uploads/staff_documents/" . $staff_id . '/' . $img_name);
                    $data_img = array('id' => $insert_id, 'document_file' => $img_name);
                    $this->leaverequest_model->addLeaveRequest($data_img);
