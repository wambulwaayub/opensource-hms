...
class Symptoms extends Admin_Controller
{
    ...
    public function deletesymtype($id)
    {
        ...
        redirect('admin/symptoms/symptomstype');
    }

    public function create()
    {

        $data['title']        = 'Add Symptoms Head';
        $category_result      = $this->symptoms_model->get();
        $data['categorylist'] = $category_result;
        $this->form_validation->set_rules('symptoms', $this->lang->line('symptoms') . " " . $this->lang->line('head'), 'trim|required|xss_clean');
    ...
    public function add()
    {
        ...
        $this->form_validation->set_rules('symptoms_title', $this->lang->line('symptoms') . " " . $this->lang->line('head'), 'trim|required|xss_clean');
        $this->form_validation->set_rules('type', $this->lang->line('symptoms') . " " . $this->lang->line('type'), 'trim|required|xss_clean');
        if ($this->form_validation->run() == false) {
            $msg = array(
    ...
    {
        $this->form_validation->set_rules('symptoms_type', $this->lang->line('symptoms') . " " . $this->lang->line('type'), 'trim|required|xss_clean');
        if ($this->form_validation->run() == false) {
    ...
    public function edit()
    {
        ...
        $this->form_validation->set_rules('symptoms_id', $this->lang->line('symptoms') . " " . $this->lang->line('head'), 'trim|required|xss_clean');
        if ($this->form_validation->run() == false) {
            $msg = array(
    ...
    public function editsymtype()
    {
        ...
        $id = $this->input->post('symptoms_id');
        $this->form_validation->set_rules('symptoms_id', $this->lang->line('symptoms') . " " . $this->lang->line('type'), 'trim|required|xss_clean');
        if ($this->form_validation->run() == false) {
            $msg = array(
                'symptoms_id' => form_error('symptoms_id'),
            );
